cd "C:\Users\ngoca\Dropbox\Pubs_since_2018\2018-Intra-Asian-FDI\Economic-Modelling\3rd submission"
capture log close
log using 3rdrevision, replace
set matsize 10000
use DATA, clear
...
/*
*convert FDI to billion usd
replace stock0=stock0/1000
replace lstock = ln(stock0)
...
test fit2 = 0 // p-value = 0.045
qui drop fit*

*=====================================HPC===========================================================

qui tobit stock0 $withfe, ll(0) vce(cluster dist)
qui predict double fitT, ystar(0,.)
...
esttab using All-Tables.rtf, star(* 0.1 ** 0.05 *** 0.01) stats (N ll_0 ll) ///
se(3) b(3) nodepvars drop ( y* *id*) noomitted title(Table 6: Results with Country Fixed Effects) compress replace style(tab) nogap
...
esttab using All-Tables.rtf, star(* 0.1 ** 0.05 *** 0.01) stats (N r2 ll_0 ll) ///
se(3) b(3) nodepvars drop ( y* ) noomitted title(Table 5: Results without Country Fixed Effects) compress append nogap
...
esttab using All-Tables.rtf, star(* 0.1 ** 0.05 *** 0.01) stats (N r2 ll_0 ll ) ///
se(3) b(3) nodepvars drop (  y* *id*) noomitted title(Table 7: Results for Pairs with at Least One Year of Positive FDI) compress append nogap
...
esttab using All-Tables.rtf, star(* 0.1 ** 0.05 *** 0.01) stats (N r2  ll_0 ll) ///
se(3) b(3) nodepvars drop (  y* *id*) noomitted title(Table 9: Results based on Braconier et al. (2005) Specification) compress append nogap
...
esttab using All-Tables.rtf, star(* 0.1 ** 0.05 *** 0.01) stats (N r2  ll_0 ll) ///
se(3) b(3) nodepvars drop (  y* *id*) noomitted title(Table 8a: Results based on Davies (2008)’ Specification, negative FDI) compress append nogap
...
esttab using All-Tables.rtf, star(* 0.1 ** 0.05 *** 0.01) stats (N r2 ll_0 ll ) ///
se(3) b(3) nodepvars drop ( y* *id*) noomitted title(Table 8b: Results based on Davies (2008)’ Specification, positive FDI) compress append nogap
...
se(3) b(3) nodepvars drop (  y* *id*) noomitted nogaps title(Table A5: Results based on Bersgtrand and Egger 2013 Specification) compress append
...
esttab using Gravity2019.rtf, star(* 0.1 ** 0.05 *** 0.01) stats (N r2 ll_0 ll ) ///
se(3) b(3) nodepvars drop (  y* *id*) noomitted nogaps title(Results without BIT and Similarity) compress append nogap
...
esttab using Gravity2019.rtf, star(* 0.1 ** 0.05 *** 0.01) stats (N r2  ll_0 ll ) ///
se(3) b(3) nodepvars drop (  y* *id*) noomitted nogaps title(Table A6: Gravity Specification) compress append nogap

* Some additional tests for the gravity specification
*1.Reset
qui ppml stock0 $withfe, cluster(dist)
qui predict fit, xb
qui predict double Pfit
qui gen fit2 = fit^2
qui ppml stock0 $withfe fit2,  cluster(dist) 
...
*PPML versus LH
hpc stock0  $withfe , a(Lfit) b(Pfit) cluster(dist) // pA = 0.201, pB = 0.998
...
se(3) b(3) nodepvars drop (  y* *id*) noomitted nogaps title(Table A5: Results based on Bersgtrand and Egger 2013 Specification and new capital variable) compress replace nogap
